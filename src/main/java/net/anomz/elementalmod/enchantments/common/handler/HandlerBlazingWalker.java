...
import net.anomz.elementalmod.enchantments.common.registries.ModBlocks;
...
import static net.anomz.elementalmod.enchantments.common.registries.ModEnchants.BLAZING_WALKER;
            ...
            for(BlockPos blockpos : BlockPos.getAllInBoxMutable(pos.add((double)(-f), -1.0D, (double)(-f)), pos.add((double)f, -1.0D, (double)f))) {
                if (blockpos.withinDistance(living.getPositionVec(), (double)f)) {
                    ...
                    if (blockstate1.isAir(worldIn, blockpos$mutable)) {
                        ...
                        if (blockstate2.getMaterial() == Material.LAVA && isFull && blockstate.isValidPosition(worldIn, blockpos) && worldIn.placedBlockCollides(blockstate, blockpos, ISelectionContext.dummy()) && !ForgeEventFactory.onBlockPlace(living, BlockSnapshot.create(worldIn.getDimensionKey(), worldIn, blockpos), Direction.UP)) {
